...
def load(req: LoadRequest) -> dict:
    ...
    cache_dir = options.get("cache_dir", "modele/cache/surya")
...
def predict_batch(req: PredictBatchRequest) -> dict:
    ...
    try:
        ...
        for idx, img_b64 in enumerate(req.images_base64):
            # id zeby nie pomieszac plikow temp przy batchowaniu
        ...
        images = []
        bboxes = []
        task_names = [state["task_name"]] * len(temp_paths)
        ...
        for path in temp_paths:
            with Image.open(path) as img:
                ...
                # Surya oczekuje bboxes dla kazdego wycinka, ale my mamy juz wycinki (linie), wiec podajemy bbox obejmujacy caly obraz.
